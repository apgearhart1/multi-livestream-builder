...
var user_options = {
  ...
  var get_webcast_reloaded_url = function(video_url, vtt_url, referer_url, force_http, force_https) {
    ...
    referer_url           = referer_url ? referer_url : window.unsafeWindow.location.href
  ...
  var process_video_url = function(video_url, video_type, vtt_url, referer_url) {
    if (!referer_url)
      referer_url = window.unsafeWindow.location.href
  ...
  var update_page_DOM = function() {
    var keep = [
      window.unsafeWindow.document.querySelector('div.site-wrapper')
    ]
  
    keep = keep.filter(function(el){return !!el})
    if (!keep.length) return
  
    while(window.unsafeWindow.document.body.childNodes.length)
      window.unsafeWindow.document.body.removeChild(window.unsafeWindow.document.body.childNodes[0])
  
    for (var i=0; i < keep.length; i++)
      window.unsafeWindow.document.body.appendChild(keep[i])
  }
  ...
  var get_hls_url = function() {
    ...
    try {
      ...
      scripts = window.unsafeWindow.document.querySelectorAll('script:not([src])')
  ...
  export var process_page = function() {
